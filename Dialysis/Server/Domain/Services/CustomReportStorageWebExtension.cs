...
namespace Dialysis.Server.Domain.Services
{
    public class CustomReportStorageWebExtension : DevExpress.XtraReports.Web.Extensions.ReportStorageWebExtension
    {
        ...
        public override byte[] GetData(string url) {
            ...
            var repList = DbContext.Reports.Select(t => t).ToList();
            if (repList != null)
            {
                var selReport = repList.Where(p => p.Name.Equals(url)).FirstOrDefault();
                if (selReport != null)
                {
                    return selReport.LayoutData;
                }
            }
                ...
                if (StaticReportsFactory.Reports.ContainsKey(url))
                {
                    using var ms = new MemoryStream();
                    using XtraReport report = StaticReportsFactory.Reports[url]();
        ...
        public override Dictionary<string, string> GetUrls() {
            ...
            var repList = DbContext.Reports.Select(t=>t.Name).ToList();
            if (repList != null)
            {
                return repList.ToDictionary<string, string>(p=>p);
            }
            return DbContext.Reports
                .ToList()
                .Select(x => x.Name)
                .Union(StaticReportsFactory.Reports.Select(x => x.Key))
